...
busybox_defconfig_source = Path("busybox.defconfig") # Die vorbereitete defconfig-Datei

firmware_clone_dir = build_dir / "firmware"
...
def create_dirs():
    ...
    os.chmod(rootfs_dir / "tmp", 0o1777) # Sticky bit für /tmp
    # Die folgenden sind typischerweise 0o755 oder 0o777, nicht 0o1777
    os.chmod(rootfs_dir / "var/log", 0o777) # Schreibbar für alle
    os.chmod(rootfs_dir / "var/run", 0o777) # Schreibbar für alle
    os.chmod(rootfs_dir / "var/tmp", 0o1777) # Sticky bit für /var/tmp
...
def create_root_init_script(rootfs_full_path: Path):
    """
    ...
    Args:
        rootfs_full_path: Das Basisverzeichnis des Rootfs (z.B. pathlib.Path("build/rootfs")).
    """
    init_script_path = rootfs_full_path / "init" # Direkt im Wurzelverzeichnis
...
"""
    ...
    try:
        # Kein .parent.mkdir() nötig, da es direkt in rootfs_full_path liegt,
...
def create_rootfs_ext4_image(rootfs_full_path: Path, image_output_path: Path, image_size_mb: int = 512):
    """
    Erstellt ein EXT4-Image aus einem gegebenen Rootfs-Verzeichnis.

    Args:
        rootfs_full_path (Path): Pfad zum Rootfs-Verzeichnis (z. B. Path("build/rootfs")).
        image_output_path (Path): Zielpfad der zu erstellenden Image-Datei (z. B. Path("build/rootfs.ext4")).
        image_size_mb (int): Größe des Images in Megabyte (Standard: 512 MB).
    """
    print(f"\n--- Erstelle EXT4-Image '{image_output_path}' ---")

    # 1. Root-Rechte prüfen
    if os.geteuid() != 0:
        print("Fehler: Diese Funktion erfordert Root-Rechte (z. B. mit sudo ausführen).")
        sys.exit(1)

    # 2. Eingabepfad prüfen
    if not rootfs_full_path.is_dir():
        print(f"Fehler: Rootfs-Verzeichnis '{rootfs_full_path}' existiert nicht oder ist kein Verzeichnis.")
        sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            print("Fallocate fehlgeschlagen, verwende dd als Fallback...")
        ...
        print(f"Formatiere '{image_output_path}' als ext4...")
        run_command(["mkfs.ext4", "-F", str(image_output_path)], desc="EXT4 formatieren", check_root=True)
        print("Formatierung abgeschlossen.")

        # 6. Image mounten
        print(f"Hänge Image '{image_output_path}' in '{temp_mount_point}' ein...")
        run_command(["mount", str(image_output_path), str(temp_mount_point)], desc="Image mounten", check_root=True)
        print("Image eingehängt.")

        # 7. Rootfs-Inhalte kopieren
        print(f"Kopiere Inhalte von '{rootfs_full_path}' nach '{temp_mount_point}'...")
        run_command(["rsync", "-a", "--info=progress2", f"{rootfs_full_path.as_posix()}/", f"{temp_mount_point.as_posix()}/"], desc="Rootfs kopieren", check_root=True)
        ...
        if temp_mount_point and temp_mount_point.is_mount():
            ...
            try:
                ...
                print(f"WARNUNG: Aushängen fehlgeschlagen: {e_umount}")
                print(f"         Manuell ausführen: sudo umount -l {temp_mount_point}")

        # 9. Mount-Verzeichnis entfernen
        if temp_mount_point and temp_mount_point.exists():
            print(f"Entferne temporäres Verzeichnis '{temp_mount_point}'...")
            try:
                shutil.rmtree(temp_mount_point)
                print("Verzeichnis entfernt.")
            except Exception as e_rmtree:
                print(f"WARNUNG: Entfernen fehlgeschlagen: {e_rmtree}")
                print("         Bitte manuell löschen.")
...
def create_rootfs_image(
    ...
    if os.geteuid() != 0:
        print("Fehler: Diese Funktion benötigt Root-Rechte (bitte mit sudo ausführen).")
        sys.exit(1)

    if not rootfs_dir.is_dir():
        print(f"Fehler: Verzeichnis '{rootfs_dir}' existiert nicht.")
        sys.exit(1)

    temp_mount = Path(tempfile.mkdtemp(prefix="rootfs_mount_"))

    try:
        print(f"Erstelle Image-Datei '{image_path}' mit {image_size_mb} MB...")
        try:
            subprocess.run(["fallocate", "-l", f"{image_size_mb}M", str(image_path)], check=True)
        except subprocess.CalledProcessError:
            print("fallocate fehlgeschlagen, wechsle zu dd...")
            subprocess.run(["dd", "if=/dev/zero", f"of={image_path}", "bs=1M", f"count={image_size_mb}"], check=True)

        print("Formatiere Image als ext4...")
        subprocess.run(["mkfs.ext4", "-F", str(image_path)], check=True)

        print(f"Mounten des Images in '{temp_mount}'...")
        subprocess.run(["mount", "-o", "loop", str(image_path), str(temp_mount)], check=True)

        print("Kopiere Rootfs-Dateien ins Image...")
        subprocess.run(["rsync", "-a", f"{rootfs_dir}/", f"{temp_mount}/"], check=True)

        print("Fertig. Rootfs-Image erfolgreich erstellt.")

    except Exception as e:
        print(f"Fehler während Erstellung: {e}")
        sys.exit(1)

    finally:
        try:
            subprocess.run(["umount", str(temp_mount)], check=True)
        except Exception as e:
            print(f"WARNUNG: Aushängen fehlgeschlagen: {e}")
            print(f"Bitte manuell ausführen: sudo umount -l {temp_mount}")
        shutil.rmtree(temp_mount, ignore_errors=True)

def create_rootfs_image2(
    rootfs_dir="build/rootfs",
    image_path="build/rootfs.ext4",
    image_size_mb=512
):
    ...
    try:
        print(f"Erstelle Image-Datei '{image_path}' mit {image_size_mb} MB...")
        try:
            subprocess.run(["sudo", "fallocate", "-l", f"{image_size_mb}M", str(image_path)], check=True)
        except subprocess.CalledProcessError:
            print("fallocate fehlgeschlagen, wechsle zu dd...")
            subprocess.run(["sudo", "dd", "if=/dev/zero", f"of={image_path}", "bs=1M", f"count={image_size_mb}"], check=True)

        print("Formatiere Image als ext4...")
        subprocess.run(["sudo", "mkfs.ext4", "-F", str(image_path)], check=True)

        print(f"Mounten des Images in '{temp_mount}'...")
        subprocess.run(["sudo", "mount", "-o", "loop", str(image_path), str(temp_mount)], check=True)

        print("Kopiere Rootfs-Dateien ins Image...")
        subprocess.run(["sudo", "rsync", "-a", f"{rootfs_dir}/", f"{temp_mount}/"], check=True)

        print("✅ Rootfs-Image erfolgreich erstellt.")
        ...
        try:
            subprocess.run(["sudo", "umount", str(temp_mount)], check=True)
        except Exception as e:
            print(f"⚠️ WARNUNG: Aushängen fehlgeschlagen: {e}")
            print(f"Bitte manuell ausführen: sudo umount -l {temp_mount}")
        shutil.rmtree(temp_mount, ignore_errors=True)


def create_bootfs_vfat_image(output_image_path: Path, firmware_clone_dir: Path, image_size_mb: int = 256):
    """
    ...
    try:
        ...
        # 3. Temporäres Mount-Verzeichnis erstellen
        temp_mount_point = Path(tempfile.mkdtemp(prefix="bootfs_mount_"))
        print(f"Temporäres Mount-Verzeichnis erstellt: {temp_mount_point}")
        ...
        shutil.copy2(output_initramfs_dir_full_path / "initrmafs.gz", temp_mount_point / "initramfs.gz")    
        print("initramfs.gz erfolgreich kopiert.")
        # 8. Erstellen von cmdline.txt und config.txt
        print("Erstelle cmdline.txt und config.txt...")
        
        # cmdline.txt Inhalt
        # root=/dev/mmcblk0p2: Gibt an, dass das Rootfs auf der zweiten Partition der SD-Karte ist.
        # init=/init: Startet das Init-Skript in Ihrem BusyBox-Rootfs.
        # rootwait: Wartet, bis das Root-Gerät verfügbar ist.
        # console=ttyS0,115200: Für serielle Konsolenausgabe.
        # console=tty1: Für lokale Konsolenausgabe.
        # cma=128M: Common Memory Area für Video/GPU (kann je nach Bedarf angepasst werden).
        # quiet: Unterdrückt überflüssige Boot-Meldungen.
        # rw: Root-Dateisystem als read-write mounten.
        cmdline_content = (
            "console=ttyS0,115200 console=tty1 "
            "root=/dev/mmcblk0p2 rootfstype=ext4 "
            "rootwait init=/init "
            "cma=128M " # Common Memory Area, wichtig für Video auf RPi5
            "quiet rw "
        ...
        # config.txt Inhalt
        # Einige Standardeinstellungen und die Kernel-Konfiguration für RPi5
        # kernel=kernel8.img: Ihr Kernel-Name.
        # arm_64bit=1: Aktiviert den 64-Bit-Modus.
        # enable_uart=1: Aktiviert UART für die serielle Konsole.
        # dtoverlay=miniuart-bt: Verlegt Bluetooth auf miniUART, um ttyS0 freizugeben.
        # dtparam=audio=on: Audio aktivieren.
        # Weitere RPi5 spezifische Optionen können hier hinzugefügt werden (z.B. overlays)
        config_content = """
...
def create_initramfs_output_dir(output_dir: Path):
    """
...
def create_initramfs():    
    run_command(["find", ".", "-print0", "|", "cpio", "--null", "-o", "--format=newc", ">", f"{output_initramfs_dir_full_path}/initrmafs.gz"], cwd=rootfs_full_path)
...
def main():
    ...
    bootfs_image_size = 256 # Größe des Bootfs-Images in MB (ca. 60-100MB für RPi-Firmware, 256MB ist sicher)
