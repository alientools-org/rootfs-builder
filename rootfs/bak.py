...
def create_rootfs_ext4_image(rootfs_full_path: Path, image_output_path: Path, image_size_mb: int = 512):
    ...
    try:
        ...
        run_command(["rsync", "-a", "--info=progress2", f"{rootfs_full_path.as_posix()}/", f"{temp_mount_point.as_posix()}/"], desc="Rootfs in Image kopieren", check_root=True)
        print("Inhalt erfolgreich kopiert.")

    except Exception as e:
        print(f"Kritischer Fehler beim Erstellen des Images: {e}")
        # We store the exception to re-raise/exit after cleanup
        error_occurred = True 
        pass # Allow finally block to execute
    finally:
        # 1. Attempt to UNMOUNT the image first
        if temp_mount_point and temp_mount_point.is_mount(): # Check if it's actually mounted
            ...
            try:
                ...
                print(f"WARNUNG: Konnte Image von '{temp_mount_point}' nicht aushängen: {e_umount}")
                print("         Bitte manuell überprüfen und ggf. 'sudo umount -l {temp_mount_point}' verwenden.")

        # 2. THEN, remove the temporary mount directory
        if temp_mount_point and temp_mount_point.exists():
            ...
            try:
                ...
                print(f"WARNUNG: Konnte temporäres Verzeichnis '{temp_mount_point}' nicht entfernen: {e_rmtree}")
                print("         Bitte manuell entfernen.")
        
        # If an error occurred in the try block, we should exit here.
        # This assumes 'error_occurred' is a flag set in the except block.
        if 'error_occurred' in locals() and error_occurred:
             sys.exit(1)
